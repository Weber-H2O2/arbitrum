 ...
 */
contract StandardArbERC721 is eERC721, Cloneable, IArbStandardToken {
     ...
     * @param _l1Address L1 address of ERC721
    ...
    function bridgeInit(address _l1Address, bytes memory _data) external override {
        ...
        (bytes memory name, bytes memory symbol) = abi.decode(_data, (bytes, bytes));
    ...
    function bridgeBurn(address account, uint256 amount) external override onlyBridge {
        require(account == msg.sender, "Ownership is not correct");
        _burn(amount);
    }
