...
import { Bridge, ArbTokenBridge__factory } from 'arb-ts/src'

import { EthERC721Bridge__factory } from '../build/types/factories/EthERC721Bridge__factory'
...
const main = async () => {
  ...
  const ethERC721Bridge = await EthERC721Bridge.deploy()

  console.log('EthERC721Bridge logic deployed to:', ethERC721Bridge.address)
  ...
  const ethERC721BridgeProxy = await L1TransparentUpgradeableProxy.deploy(
    ethERC721Bridge.address,
    l1ProxyAdmin.address,
    '0x'
  )
  await ethERC721BridgeProxy.deployed()

  console.log('L1 proxy bridge at', ethERC721BridgeProxy.address)
  ...
  // console.log("inbox after init", await ethERC721BridgeConnectedAsProxy.inbox())
  console.log('Proxies have been initted')

  const contracts = JSON.stringify({
    ...deployments,
    ethERC721Bridge: ethERC721BridgeProxy.address,
    arbToken721Bridge: arbTokenBridgeProxy.address,
    inbox: inboxAddress,
  })
  ...
  //   ethERC721Bridge.deployTransaction.hash
  // )
